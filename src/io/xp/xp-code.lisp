...
#-(or :symbolics :lucid :franz-inc :cmu :sbcl)(eval-when (eval load compile)
...
(defun structure-type-p (x) (and (symbolp x) (get x 'common-lisp::%structure-definition)))
#+:sbcl
(defun structure-type-p (x) (and (symbolp x) (subtypep x 'structure-object)))
(defun output-width     (&optional (s *standard-output*)) (declare (ignore s)) nil)
(defun output-position  (&optional (s *standard-output*)) (common-lisp::charpos s)) )
...
(common-lisp:defstruct (pprint-dispatch (:conc-name nil) (:copier nil))
...
(common-lisp:defstruct (entry (:conc-name nil))
...
(common-lisp:defstruct (xp-structure (:conc-name nil) (:print-function describe-xp))
...
(defun describe-xp (xp s depth) 
  ...
  (common-lisp:format s "#<XP stream ")
  (if (not (base-stream xp))
      (common-lisp:format s "not currently in use")
      (common-lisp:format s "outputting to ~S" (base-stream xp)))
  (when (base-stream xp)
    (common-lisp:format s "~&buffer= ~S" (subseq (buffer xp) 0 (max (buffer-ptr xp) 0)))
    (when (not *describe-xp-streams-fully*) (common-lisp:princ " ..." s))
    (when *describe-xp-streams-fully*
      (common-lisp:format s "~&   pos   _123456789_123456789_123456789_123456789")
      (common-lisp:format s "~&depth-in-blocks= ~D linel= ~D line-no= ~D line-limit= ~D"
		   (depth-in-blocks xp) (linel xp) (line-no xp) (line-limit xp))
      (when (or (char-mode xp) (not (zerop (char-mode-counter xp))))
	(common-lisp:format s "~&char-mode= ~S char-mode-counter= ~D"
		     (char-mode xp) (char-mode-counter xp)))
      (unless (minusp (block-stack-ptr xp))
	(common-lisp:format s "~&section-start")
	(do ((save (block-stack-ptr xp)))
	    ((minusp (block-stack-ptr xp)) (setf (block-stack-ptr xp) save))
	  (common-lisp:format s " ~D" (section-start xp))
	  (pop-block-stack xp)))
      (common-lisp:format s "~&linel= ~D charpos= ~D buffer-ptr= ~D buffer-offset= ~D"
		   (linel xp) (charpos xp) (buffer-ptr xp) (buffer-offset xp))
      (unless (minusp (prefix-stack-ptr xp))
	(common-lisp:format s "~&prefix= ~S"
		     (subseq (prefix xp) 0 (max (prefix-ptr xp) 0)))
	(common-lisp:format s "~&suffix= ~S"
		     (subseq (suffix xp) 0 (max (suffix-ptr xp) 0))))
      (unless (> (Qleft xp) (Qright xp))
	(common-lisp:format s "~&ptr type         kind           pos depth end offset arg")
	(do ((p (Qleft xp) (Qnext p))) ((> p (Qright xp)))
	  (common-lisp:format s "~&~4A~13A~15A~4A~6A~4A~7A~A"
		...
		(Qarg xp p)))))
      (unless (minusp (prefix-stack-ptr xp))
	(common-lisp:format s "~&initial-prefix-ptr prefix-ptr suffix-ptr non-blank start-line")
	(do ((save (prefix-stack-ptr xp)))
	    ((minusp (prefix-stack-ptr xp)) (setf (prefix-stack-ptr xp) save))
	  (common-lisp:format s "~& ~19A~11A~11A~10A~A"
  ...
  (let ((*print-pprint-dispatch* pprint-dispatch) (*print-right-margin* right-margin)
    ...
    (cond ((or (xp-structure-p stream) pretty)
	  ...
	  (T (remf pairs :dispatch) (remf pairs :right-margin)
	     ...
	     (apply #'common-lisp:write object pairs))))
  object)
...
(defun basic-write (object stream)
  (cond ((xp-structure-p stream) (write+ object stream))
	...
	(T (common-lisp:write object :stream stream))))
  ...
  (cond ((stringp stream)
	...
	(T (if (eq stream T) (setq stream *standard-output*))
	   ...
	   (cond ((not (stringp string-or-fn))
		 ...
		 (T (apply #'common-lisp:format stream string-or-fn args)))
	   nil)))
...
(defun write-char (char &optional (stream *standard-output*))
  ...
  (if (xp-structure-p stream)
      ...
      (common-lisp:write-char char stream))
  char)
...
(defun write-string (string &optional (stream *standard-output*)
  ...
  (if (xp-structure-p stream)
      ...
      (common-lisp:write-string string stream :start start :end end))
  string)
...
(defun write-line (string &optional (stream *standard-output*)
  ...
  (if (xp-structure-p stream)
      ...
      (common-lisp:write-line string stream :start start :end end))
  string)

...
(defun fresh-line (&optional (stream *standard-output*))
  ...
  (cond ((xp-structure-p stream)
	 ...
	 (when (not (zerop (LP<-BP stream)))
	   ...
	   T))
	(T (common-lisp:fresh-line stream))))
...
(defun finish-output (&optional (stream *standard-output*))
  ...
  (common-lisp:finish-output stream)
  nil)
...
(defun force-output (&optional (stream *standard-output*))
  ...
  (when (xp-structure-p stream)
    (attempt-to-output stream T T)
    (setq stream (base-stream stream)))
  (common-lisp:force-output stream)
  nil)
...
(defun clear-output (&optional (stream *standard-output*))
  ...
  (when (xp-structure-p stream)
    (let ((*locating-circularities* 0)) ;hack to prevent visible output
      (attempt-to-output stream T T)
      (setq stream (base-stream stream))))
  (common-lisp:clear-output stream)
  nil)

  ...
  (let* ((struct-name (if (consp name) (car name) name))
    ...
    (cond (printer
	   `(eval-when (eval load compile)
	      (common-lisp:defstruct ,name ,@ body)
...
(defmacro def-format-handler (char args &body body)
  (let ((name (intern (common-lisp:format nil "FORMAT-~A" char) (find-package "XP"))))
    `(eval-when (eval load compile)
...
(defun using-format (xp string &rest args)
  (let ((result (apply #'common-lisp:format nil string args)))
...
(def-format-handler #\| (start end) (declare (ignore end))
  (multiple-chars start #.(aref (common-lisp:format nil "~|") 0)))
...
(defun pretty-non-vector (xp array)
  (let* ((bottom (1- (array-rank array)))
	 ...
	 (*prefix* (common-lisp:format nil "#~DA(" (1+ bottom))))
...
(common-lisp:defstruct bq-struct code data)
...
(set-pprint-dispatch+ '(cons (member define-setf-method)) #'defun-like '(0) *IPD*) 
(set-pprint-dispatch+ '(cons (member common-lisp:defstruct)) #'block-like '(0) *IPD*) 
  ...
  (let ((scl:*print-pretty-printer* #'default-system-printing))
	...
	(unless (scl:ignore-errors (format stream (formatter "~w") object) t)
	  (let ((*print-pretty* nil))
	    (common-lisp:princ "#<Cannot pretty print " stream)
	    (unless (scl:ignore-errors (common-lisp:write object :stream stream) t)
	      (common-lisp:princ "#<Cannot print value of " stream)
	      (common-lisp:princ (sys:%p-pointer object) stream)
	      (common-lisp:princ ">" stream)))
	  (common-lisp:princ ">" stream))))
  object)
...
(defun re-pretty-print-in-full (ignore)
  ...
  (let ((*print-lines* nil)
	(*print-length* nil)
	(*print-level* nil))
    (common-lisp:fresh-line tv:selected-window)
  ...
  (when (and (zl:send tv:selected-window :interactor-p)
    ...
    (common-lisp:terpri tv:selected-window)
    (if (stringp si:*cp-prompt*) (common-lisp:princ si:*cp-prompt* tv:selected-window)
